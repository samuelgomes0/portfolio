...
function ProjectCard({ title, role, description, stack, links }: ProjectCardProps) {
  ...
  const truncated = shouldTruncate ? description.slice(0, DESCRIPTION_LIMIT).trimEnd() + '…' : description;
    ...
    <Card className="flex h-full flex-col overflow-hidden rounded-md p-4 shadow-none transition-shadow duration-200 hover:shadow-xs">
      <div className="flex grow flex-col">
        <CardHeader className="mb-2 p-0">
          <TypographySmall text={title} className="text-base font-semibold text-foreground" />
          ...
          <div>
            ...
            {shouldTruncate && (
              <Button
                ...
                className={cn(
                  'mt-1 h-auto px-0 py-0.5 text-xs font-medium text-muted-foreground',
